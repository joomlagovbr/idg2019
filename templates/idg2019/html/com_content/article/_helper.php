...
class TemplateContentArticleHelper {
	...
	static function customImg( $introimage = '' )
	{
		//Utilizado para pegar imagem detro do artigo.
		/*preg_match('/<img(.*?)>/', $introimage , $matches);
		$img = (isset($matches[1]))? $matches[1] : '';
	
		if($img != ""){
			
			$parametros = preg_replace('/src=\"(.*?)\"/', '', $img);
			preg_match('/src=\"(.*?)\"/', $img, $indice);
			$end_img 	= $indice[1];					
			$output = JURI::root() . $end_img;			
			return $output;
		}*/
		//Utilizado para pegar imagem do campo introimage da aba imagens e links
		if($introimage){
			$output = JURI::root().$introimage;
			return $output;
		}
		else{
